...
key_reg_obj, key_progress, key_calc, key_smallstat, key_tabc, key_tabt = "reg_object", "progress", "calc", "smallstat", "ch_data", "tool_data"
key_list = [key_reg_obj, key_progress, key_calc, key_smallstat, key_tabc, key_tabt]
for x in key_list:
...
with st.container(key="main_content", width=1800):
    with st.container():
        col_left, col_mid, col_right = st.columns([1000, 400, 400])
        with col_left:
            ore.register_object(key_reg_obj, key_list_reg, arciv, negotiator, DIRECTORIES, DATAPATH, data_options, TERMS, attempts)
        with col_mid:
    ...
    with st.container(vertical_alignment="center"):
        col_left, col_mid, col_right = st.columns([1000, 300, 500])
        with col_left:
            height = prog.progress_meter(key_progress, key_list_meter, arciv, negotiator, DATAPATH, TERMS, attempts)
        with col_mid:
